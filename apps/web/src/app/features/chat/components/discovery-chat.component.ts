...
@Component({
  ...
  styles: [`
    ...
    .dmsg-assistant :first-child { margin-top: 0; }
    .dmsg-assistant :last-child { margin-bottom: 0; }
    :host ::ng-deep .dmsg-assistant p { margin: 4px 0; }
    :host ::ng-deep .dmsg-assistant a { color: #60A5FA; text-decoration: underline; }
    :host ::ng-deep .dmsg-assistant a:hover { color: #93C5FD; }
    :host ::ng-deep .dmsg-assistant ul,
    :host ::ng-deep .dmsg-assistant ol { margin: 4px 0; padding-left: 16px; }
    :host ::ng-deep .dmsg-assistant li { margin: 2px 0; }
    :host ::ng-deep .dmsg-assistant strong { color: #FAFAFA; }
    :host ::ng-deep .dmsg-assistant h3 { font-size: 12px; font-weight: 600; margin: 6px 0 2px; color: #FAFAFA; }
    :host ::ng-deep .dmsg-assistant code {
      background: #242424; padding: 1px 4px; border-radius: 3px; font-size: 11px;
    }
    .dmsg-streaming {
      border-color: #3B82F6;
    }
    ...
    .discovery-send-btn:hover { background: #2563EB; }
    .discovery-send-btn:disabled { background: #242424; color: #4A4A4A; cursor: not-allowed; }
    .discovery-send-btn svg { width: 14px; height: 14px; }
    @keyframes pulse-dot {
      0%, 100% { opacity: 0.4; }
      50% { opacity: 1; }
    }
  ...
  template: `
    <div class="discovery-container">
      <div class="discovery-header" (click)="toggleExpanded()">
        ...
        <svg class="toggle-icon" [class.expanded]="isExpanded$()" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="discovery-messages" #messagesArea>
            ...
            @for (msg of messages$(); track $index) {
              @if (msg.role === 'user') {
                ...
                <div class="dmsg-assistant" [class.dmsg-streaming]="$index === messages$().length - 1 && isStreaming$()" [innerHTML]="msg.html ?? msg.content"></div>
              }
            }
...
export class DiscoveryChatComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.unsubError = this.chatWsService.onDiscoveryError((_data) => {
      ...
      this.messages$.update((msgs) => [
        ...msgs,
        { role: 'assistant' as const, content: 'Greška pri obradi. Pokušajte ponovo.', html: '<p style="color:#EF4444;">Greška pri obradi. Pokušajte ponovo.</p>' },
      ]);
    });
  }
  ...
  sendMessage(): void {
    ...
    this.messages$.update((msgs) => [...msgs, { role: 'assistant' as const, content: '', html: '' }]);
