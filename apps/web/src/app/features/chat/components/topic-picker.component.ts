...
@Component({
  ...
  styles: [`
    ...
    .picker-header {
      padding: 16px 20px 12px; border-bottom: 1px solid #2A2A2A;
    }
    .picker-title {
      font-size: 15px; font-weight: 600; color: #FAFAFA; margin-bottom: 12px;
    }

    .search-input {
      width: 100%; background: #0D0D0D; border: 1px solid #2A2A2A;
      border-radius: 8px; padding: 10px 12px 10px 36px;
      color: #FAFAFA; font-size: 13px; font-family: inherit;
    }
    ...
    .result-item:hover { background: #242424; }

    .result-label {
      font-size: 13px; color: #FAFAFA;
    }
    .result-path {
      font-size: 11px; color: #4A4A4A; margin-top: 2px;
    }
    ...
    .skip-btn {
      ...
      padding: 8px 16px; color: #A1A1A1; font-size: 13px; cursor: pointer;
    }
    .skip-btn:hover { color: #FAFAFA; border-color: #4A4A4A; }
    ...
    <div class="overlay" (click)="onOverlayClick($event)">
      <div class="picker-panel" (click)="$event.stopPropagation()">
        <div class="picker-header">
          <div class="picker-title">Choose a topic for your conversation</div>
        ...
        <div class="picker-footer">
          ...
          <button class="skip-btn" (click)="topicSelected.emit(null)">Skip — General Chat</button>
        </div>
      </div>
    </div>
  `,
  ...
  private async loadTopLevelNodes(): Promise<void> {
    ...
    try {
      ...
      const fullRes = await firstValueFrom(
        this.http.get<{ data: CurriculumNode[] }>(this.apiUrl)
      );
