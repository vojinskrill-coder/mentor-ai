...
@Component({
  ...
  styles: [`
    :host { display: block; }
    .page { min-height: 100vh; background: #0D0D0D; color: #FAFAFA; font-family: 'Inter', system-ui, sans-serif; }

    /* Header */
    .page-header { border-bottom: 1px solid #2A2A2A; background: #1A1A1A; }
    .header-inner { max-width: 800px; margin: 0 auto; padding: 16px 24px; display: flex; align-items: center; gap: 12px; }
    .back-link { color: #6B6B6B; text-decoration: none; display: flex; align-items: center; }
    .back-link:hover { color: #FAFAFA; }
    .back-link svg { width: 20px; height: 20px; }
    .header-icon svg { width: 24px; height: 24px; color: #3B82F6; }
    .page-title { font-size: 20px; font-weight: 600; }

    /* Main Content */
    .page-content { max-width: 800px; margin: 0 auto; padding: 32px 24px; }

    /* Loading */
    .loading-center { display: flex; align-items: center; justify-content: center; padding: 48px 0; }
    @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
    .spinner { width: 32px; height: 32px; border: 3px solid #2A2A2A; border-top-color: #3B82F6; border-radius: 50%; animation: spin 0.8s linear infinite; }

    /* Section Card */
    .section-card { background: #1A1A1A; border: 1px solid #2A2A2A; border-radius: 12px; padding: 24px; margin-bottom: 24px; }
    .section-title { font-size: 18px; font-weight: 600; margin-bottom: 8px; }
    .section-desc { font-size: 14px; color: #A1A1A1; margin-bottom: 24px; line-height: 1.5; }

    /* Provider Grid */
    .provider-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin-bottom: 24px; }
    .provider-btn { display: flex; align-items: center; gap: 12px; padding: 16px; border-radius: 10px; border: 2px solid #2A2A2A; background: transparent; color: #FAFAFA; cursor: pointer; text-align: left; transition: border-color 0.2s; }
    .provider-btn:hover { border-color: rgba(59, 130, 246, 0.5); }
    .provider-btn.selected { border-color: #3B82F6; background: rgba(59, 130, 246, 0.05); }
    .provider-icon { width: 40px; height: 40px; border-radius: 8px; display: flex; align-items: center; justify-content: center; background: rgba(59, 130, 246, 0.1); }
    .provider-icon svg { width: 20px; height: 20px; color: #3B82F6; }
    .provider-info {}
    .provider-name { font-size: 15px; font-weight: 500; }
    .provider-desc { font-size: 12px; color: #A1A1A1; margin-top: 2px; }

    /* Form Fields */
    .form-group { margin-bottom: 16px; }
    .form-label { display: block; font-size: 13px; font-weight: 500; margin-bottom: 6px; color: #FAFAFA; }
    ...
    .form-select option { background: #1A1A1A; color: #FAFAFA; }
    ...
    .btn-outline:hover:not(:disabled) { background: #242424; }
    ...
    /* Validation Status */
    .validation-status { display: flex; align-items: center; gap: 8px; font-size: 13px; margin-top: 12px; }
    .validation-status svg { width: 16px; height: 16px; flex-shrink: 0; }
    .validation-success { color: #22C55E; }
    .validation-error { color: #EF4444; }

    /* Resource Info */
    .resource-box { background: #242424; border-radius: 8px; padding: 12px 16px; margin-top: 12px; }
    .resource-title { font-size: 13px; font-weight: 500; margin-bottom: 8px; }
    .resource-list { list-style: none; padding: 0; margin: 0; }
    .resource-list li { font-size: 13px; color: #A1A1A1; padding: 2px 0; }

    /* Fallback Header */
    .fallback-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; }
    .checkbox-label { display: flex; align-items: center; gap: 8px; font-size: 13px; color: #A1A1A1; cursor: pointer; }
    .checkbox-label input[type="checkbox"] { accent-color: #3B82F6; width: 16px; height: 16px; cursor: pointer; }

    /* Save Row */
    .save-row { display: flex; align-items: center; justify-content: flex-end; gap: 16px; margin-top: 8px; }
    .msg-success { font-size: 13px; color: #22C55E; }
    .msg-error { font-size: 13px; color: #EF4444; }
  `],
  template: `
    <div class="page">
          ...
          <div class="section-card">
            ...
            @if (primaryProviderType$() === 'OPENROUTER') {
              ...
              @if (primaryValidation$()) {
                <div class="validation-status" [class.validation-success]="primaryValidation$()!.valid" [class.validation-error]="!primaryValidation$()!.valid">
                  ...
                  <div class="resource-box">
                    ...
                    <ul class="resource-list">
                      @if (primaryValidation$()!.resourceInfo!.gpuRequired) {
                        <li>GPU: {{ primaryValidation$()!.resourceInfo!.gpuMemoryGb }} GB VRAM recommended</li>
                      }
...
export class LlmConfigComponent implements OnInit {
  ...
  validatePrimaryProvider(): void {
    ...
    const apiKey = (type === LlmProviderType.OPENROUTER || type === LlmProviderType.OPENAI) ? this.primaryApiKey : undefined;
    const endpoint = type === LlmProviderType.LOCAL_LLAMA ? this.primaryEndpoint : undefined;
